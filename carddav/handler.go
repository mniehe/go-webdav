...
type Config struct {
	...
	// MaxCollectionScan and MaxCollectionBytes bound how much of one address
	// book a single request may materialise. Every request that reads a
	// collection — a query, a Depth:1 PROPFIND, an initial sync — lists it
	// whole before answering, so without a ceiling a hostile client could force
	// the entire collection into memory and through the parser before any
	// per-result limit applies. A listing past either ceiling fails with 507
	// rather than a partial success. Zero uses the defaults below.
...
// personal address book reaches it, small enough that a hostile collection is
// stopped before it exhausts memory.
const (
...
func NewHandler(b Backend, cfg Config) (*Handler, error) {
	switch {
	case b == nil:
		return nil, fmt.Errorf("carddav: nil backend")
	case cfg.Authenticate == nil:
		return nil, fmt.Errorf("carddav: Config.Authenticate is required")
	case cfg.WWWAuthenticate == "":
		return nil, fmt.Errorf("carddav: Config.WWWAuthenticate is required, or a client cannot learn how to authenticate")
	}
	if cfg.Routes == nil {
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "REPORT" {
		...
		if err := h.adapter.handleReport(w, r); err != nil {
			internal.ServeError(w, err)
		}
		return
	}
...
// whose PROPFIND answers current-user-principal and addressbook-home-set.
...
func (h *Handler) redirectToPrincipal(w http.ResponseWriter, r *http.Request) bool {
	...
	if path != "/.well-known/carddav" && path != "/" {
		return false
	}
	if path == "/" {
