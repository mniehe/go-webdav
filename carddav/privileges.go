...
// The privileges this handler reports, all RFC 3744's in the DAV: namespace.
// CardDAV defines no privilege of its own: a contact has no busy-time shadow,
// so there is no CardDAV analogue of read-free-busy.
var (
...
// tree itself.
func privilegesFor(perms AddressBookPermissions, caps capabilities) []xml.Name {
	var names []xml.Name
	...
	settings := perms.UpdateSettings && caps.updatesBooks
...
func capabilitiesOf(b Backend) capabilities {
	...
	_, caps.createsBooks = b.(BookCreator)
	_, caps.updatesBooks = b.(BookUpdater)
	_, caps.deletesBooks = b.(BookDeleter)
