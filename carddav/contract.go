...
func (a *adapter) getAddressBook(ctx context.Context, ref AddressBookRef) (AddressBook, error) {
	cal, err := a.backend.GetAddressBook(ctx, ref)
	if err != nil {
		return AddressBook{}, err
	}
	if cal.ID == "" {
		return AddressBook{}, contractViolation("calendar %q came back with a zero BookID", ref.Book)
	}
	if cal.Name != ref.Book {
		return AddressBook{}, contractViolation("calendar %q came back named %q", ref.Book, cal.Name)
	}
	return cal, nil
}
...
func (a *adapter) listAddressBooks(ctx context.Context, account AccountID) ([]AddressBook, error) {
	cals, err := a.backend.ListAddressBooks(ctx, account)
	if err != nil {
		return nil, err
	}
	...
	for i := range cals {
		...
		if cals[i].ID == "" {
			return nil, contractViolation("calendar %q came back with a zero BookID", cals[i].Name)
		}
		if seen[cals[i].Name] {
...
func (a *adapter) listItems(ctx context.Context, ref AddressBookRef) ([]Item, Revision, error) {
	var items []Item
	var scanned int64
	overrun := false
	...
	for i := range items {
		if items[i].Name.IsZero() {
			return nil, 0, contractViolation("calendar %q listed an item with an invalid name", ref.Book)
		}
		if seen[items[i].Name] {
			return nil, 0, contractViolation("calendar %q listed item %q twice", ref.Book, items[i].Name)
		}
