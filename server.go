...
type Handler struct {
	...
	// DisablePathValidation passes client-supplied paths to the FileSystem
	// unchecked. By default the server refuses a non-canonical path, one hiding a
	// separator once decoded, and a COPY or MOVE Destination on another origin.
	//
	// Set it only if the FileSystem makes equivalent checks itself; it then owns
	// the consequences of not doing so.
	DisablePathValidation bool

	// MaxResponsePropBytes bounds the property names one response echoes back,
	// summed over every resource it reports. A request naming the most properties
	// the other bounds allow otherwise buys a response proportional to the
	// collection size. Zero uses the package default; a negative value removes the
	// bound, leaving the FileSystem to survive whatever is asked of it.
	MaxResponsePropBytes int

	// MaxResponseBytes bounds what one multistatus may put on the wire. It is the
	// backstop for content whose size the request does not predict, which the
	// property echo bound above cannot see. Zero uses the package default; a
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	hh := internal.Handler{Backend: &b, DisablePathValidation: h.DisablePathValidation, MaxResponsePropBytes: h.MaxResponsePropBytes, MaxResponseBytes: h.MaxResponseBytes}
	hh.ServeHTTP(w, r)
}

// HTTPError is an error carrying the HTTP status code that describes it. The
// client returns one for every non-2xx response and a Backend returns one to
// choose the status the server answers with, so a caller recovers the code with
// errors.As rather than by matching on message text:
//
...
//	}
//
// Err is the cause and is reachable through errors.Is and errors.Unwrap. A
// server never sends a Backend's cause text to the client — only the status code
// and the library's own message reach the wire — so the cause serves logs and
// callers on this side of the connection, not the response body.
type HTTPError = internal.HTTPError

// NewHTTPError returns an error associated with an HTTP status code, and
// optionally the error that led to it. A Backend returns one to convey some
// semantics to the server: 404 not found, 403 access denied, and so on.
//
// The cause is not disclosed to the client; see HTTPError. Only the status code
// is protocol-facing.
...
func (b *backend) Options(r *http.Request) (caps, allow []string, err error) {
...
func (b *backend) HeadGet(w http.ResponseWriter, r *http.Request) error {
	...
	if rs, ok := f.(io.ReadSeeker); ok {
		...
		io.Copy(w, f) //nolint:errcheck // best-effort response write
	}
	return nil
}

func (b *backend) PropFind(r *http.Request, propfind *internal.PropFind, depth internal.Depth, emit func(*internal.Response) error) error {
	fi, err := b.FileSystem.Stat(r.Context(), r.URL.Path)
	if err != nil {
		return err
	}
	...
	if depth == internal.DepthZero || !fi.IsDir {
		resp, perr := b.propFindFile(propfind, fi)
		if perr != nil {
			return perr
		}
		return emit(resp)
	...
	for i := range children {
		resp, perr := b.propFindFile(propfind, &children[i])
		if perr != nil {
			return perr
		}
		if eerr := emit(resp); eerr != nil {
			return eerr
		}
	}
	return nil
...
func ServePrincipal(w http.ResponseWriter, r *http.Request, options *ServePrincipalOptions) {
	...
	case http.MethodOptions:
		caps := make([]string, 0, 2+len(options.Capabilities))
		caps = append(caps, "1", "3")
...
func servePrincipalPropfind(w http.ResponseWriter, r *http.Request, options *ServePrincipalOptions) error {
	...
	if err := internal.BoundPropNames(propfind.Prop); err != nil {
		return err
	}
	principalPath := options.CurrentUserPrincipalPath
	props := map[xml.Name]internal.PropFindFunc{
		...
		internal.PrincipalURLName: internal.PropFindValue(&internal.PrincipalURL{
			Href: internal.Href{Path: principalPath},
		}),
		internal.PrincipalCollectionSetName: internal.PropFindValue(&internal.PrincipalCollectionSet{
			Hrefs: []internal.Href{{Path: internal.PrincipalCollectionPath(principalPath)}},
		}),
	}
