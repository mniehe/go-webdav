...
func DiscoverContextURL(ctx context.Context, service, domain string) (string, error) {
	...
	case 1:
		...
		davPath = strings.TrimPrefix(record, "path=")
		if davPath == "" {
...
func (c *Client) NewRequest(method, p string, body io.Reader) (*http.Request, error) {
	return http.NewRequest(method, c.ResolveHref(p).String(), body)
}

func (c *Client) NewXMLRequest(method, p string, v interface{}) (*http.Request, error) {
	var buf bytes.Buffer
	buf.WriteString(xml.Header) //nolint:errcheck // bytes.Buffer.WriteString never returns an error
...
func (c *Client) Do(req *http.Request) (*http.Response, error) {
	...
	if resp.StatusCode/100 != 2 {
		...
		t, _, _ := mime.ParseMediaType(contentType) //nolint:errcheck // a malformed media type yields an empty t, handled by the branches below
		if t == "application/xml" || t == "text/xml" {
			...
			var buf bytes.Buffer
			io.Copy(&buf, &lr) //nolint:errcheck // best-effort read of a truncated error body
			resp.Body.Close()
...
func (c *Client) PropFind(ctx context.Context, p string, depth Depth, propfind *PropFind) (*MultiStatus, error) {
	req, err := c.NewXMLRequest("PROPFIND", p, propfind)
	if err != nil {
		return nil, err
	}
...
func (c *Client) Options(ctx context.Context, p string) (classes, methods map[string]bool, err error) {
	req, err := c.NewRequest(http.MethodOptions, p, nil)
	if err != nil {
		return nil, nil, err
	}
...
func (c *Client) SyncCollection(ctx context.Context, p, syncToken string, level Depth, limit *Limit, prop *Prop) (*MultiStatus, error) {
	...
	req, err := c.NewXMLRequest("REPORT", p, &q)
	if err != nil {
		return nil, err
	}
