...
// build an arbitrarily long URL out of it. Generous for a calendar or item
...
func ParseSegment(s string) (Segment, error) {
	switch {
	case s == "":
		return Segment{}, fmt.Errorf("caldav: empty path segment")
	case s == "." || s == "..":
		return Segment{}, fmt.Errorf("caldav: %q is a dot segment", s)
	case len(s) > maxSegmentLen:
		return Segment{}, fmt.Errorf("caldav: path segment is %d bytes, over the %d limit", len(s), maxSegmentLen)
	case !utf8.ValidString(s):
		return Segment{}, fmt.Errorf("caldav: path segment is not valid UTF-8")
	}
	for _, r := range s {
		if r == '/' {
			return Segment{}, fmt.Errorf("caldav: path segment %q contains a slash", s)
		}
		if unicode.IsControl(r) {
			return Segment{}, fmt.Errorf("caldav: path segment %q contains a control character", s)
		}
	}
...
// CalendarRef names one calendar. Account is the account that owns it, which is
...
// ItemRef names one item within a calendar.
type ItemRef struct {
