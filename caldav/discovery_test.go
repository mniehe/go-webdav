...
func TestWellKnownRedirectsToThePrincipal(t *testing.T) {
	...
	for _, method := range []string{http.MethodGet, "PROPFIND", http.MethodOptions} {
		t.Run(method, func(t *testing.T) {
			w := do(h, method, "/.well-known/caldav")
...
func TestDiscoveryChallengesBeforeRedirecting(t *testing.T) {
	...
	w := do(h, "PROPFIND", "/.well-known/caldav")
...
func TestDiscoveryFlowReachesTheCalendarHome(t *testing.T) {
	h := handlerFor(t, newStore(t), caldav.Config{})

	// The whole bootstrap, as a client walks it: well-known, redirect,
	// current-user-principal, calendar-home-set — all landing on one URL.
	target := do(h, "PROPFIND", "/.well-known/caldav").Header().Get("Location")
	if target == "" {
	...
	for _, want := range []string{"current-user-principal", "calendar-home-set"} {
...
// layout might.
type rootClaimingRoutes struct{ caldav.Routes }

func (r rootClaimingRoutes) Parse(ctx context.Context, path string) (caldav.Resource, error) {
	if path == "/" {
		return caldav.AccountResource("alice"), nil
	}
	return r.Routes.Parse(ctx, path)
}

func TestRootIsServedWhenRoutesClaimIt(t *testing.T) {
	h := handlerFor(t, newStore(t), caldav.Config{Routes: rootClaimingRoutes{caldav.DefaultRoutes("")}})
