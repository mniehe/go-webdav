...
var (
	workScope     = calendarScope("calendar-1")
	personalScope = calendarScope("calendar-2")
)

func TestCalendarScopeFollowsTheIncarnation(t *testing.T) {
	if workScope == personalScope {
		t.Fatal("two calendars share a scope")
	}
	if calendarScope("calendar-1") != workScope {
		t.Error("the same calendar hashed to two different scopes")
	}

	// The point of scoping by ID rather than by owner and name: a calendar
	// deleted and recreated at the same reference is a different calendar, its
	// revisions start over, and validators issued before the deletion must stop
	// matching.
	recreated := calendarScope("calendar-3")
	if recreated == workScope {
		t.Error("a recreated calendar reused the deleted one's scope, so a stale If-Match still matches")
	}
}
...
func TestETagRoundTrip(t *testing.T) {
	...
	if _, ok := parseETag(personalScope, tag); ok {
		t.Error("a tag issued for one calendar parsed against another; a client copying one across would silently satisfy a precondition")
	}
}
...
func TestSyncTokenRoundTrip(t *testing.T) {
	...
	if _, ok := parseSyncToken(personalScope, token); ok {
		t.Error("a token issued for one calendar parsed against another, so a client could sync one calendar from another's position")
	}
}
