...
// copying a whole calendar is not offered.
//
...
// the source calendar's permissions, and the pair of destination permissions
// the backend selects between. The source's conditional headers and the
// Overwrite refusal both travel as preconditions, checked inside the backend's
// transaction like every other compare-and-mutate.
func (a *adapter) transfer(r *http.Request, dest *internal.Href, overwrite, move bool) (bool, error) {
	...
	if !acc.calendar.ViewDetails {
		return false, denyOperation("read the items in this calendar")
	}
	if move && !acc.calendar.DeleteItems {
		return false, denyOperation("move items out of this calendar")
	}
	...
	if err != nil || dst.Kind != KindItem {
		...
		return false, internal.HTTPErrorf(http.StatusConflict, "caldav: Destination cannot hold an item")
	}

	dstPerms := acc.calendar
	if dst.CalendarRef() != acc.CalendarRef() {
		perms, permErr := a.backend.CalendarPermissions(ctx, actorFrom(ctx), dst.CalendarRef())
		if permErr != nil {
			return false, fmt.Errorf("caldav: reading destination permissions: %w", permErr)
		}
	...
	srcCal, err := a.getCalendar(ctx, acc.CalendarRef())
	if err != nil {
		return false, backendError(err)
	}
	source, err := preconditionsFrom(calendarScope(srcCal.ID), r.Header.Get("If-Match"), r.Header.Get("If-None-Match"))
	if err != nil {
		return false, internal.HTTPErrorf(http.StatusBadRequest, "caldav: %v", err)
	}
	...
	if err != nil {
		...
		// The source calendar was resolved above, so a missing parent can only
		// be the destination's — RFC 4918 §9.8.5 answers that with 409.
		if errors.Is(err, ErrParentNotFound) {
			return false, internal.HTTPErrorf(http.StatusConflict, "caldav: the Destination calendar does not exist")
		}
		return false, backendError(err)
	}
