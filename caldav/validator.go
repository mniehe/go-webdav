...
// calendarScope distinguishes validators issued for different calendars, so a
// tag or token copied from one cannot match a same-numbered revision in
// another.
//
// It hashes the CalendarID rather than the reference. That is what makes a
...
func calendarScope(id CalendarID) uint64 {
...
func preconditionsFrom(scope uint64, ifMatch, ifNoneMatch string) (Preconditions, error) {
	...
	case ifMatch != "" && ifNoneMatch != "":
		return Preconditions{}, fmt.Errorf("caldav: If-Match and If-None-Match cannot be combined")
	case ifMatch == "*":
